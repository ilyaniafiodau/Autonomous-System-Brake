                     ...
                     bool* const              loopback)
{
    ...
    if (poll_result > 0)
    {
        ...
        // Убрана проверка типа идентификатора. Теперь принимаются и расширенные 29-ти битные, 
        // и стандартные 11-ти битные
        const bool valid = ((sockcan_frame.can_id & CAN_ERR_FLAG) == 0) &&  // Not RTR frame
                           ((sockcan_frame.can_id & CAN_RTR_FLAG) == 0);    // Not error frame
        if (!valid)
        {
